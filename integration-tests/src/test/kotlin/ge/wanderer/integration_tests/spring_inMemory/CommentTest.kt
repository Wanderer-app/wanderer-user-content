...
class CommentTest(
    ...
    fun findsCommentById() {
        val response = commentService.findById(1)
    ...
    fun canBeRepliedTo() {
        ...
        val commentsReplies = commentService.listComments(1, DEFAULT_LISTING_PARAMS)
        assertEquals(5, commentsReplies.resultSize)

        val comment = commentService.findById(commentId).data!!
    ...
    fun canBeRated() {
        ...
        var comment = commentService.findById(commentId).data!!
        assertEquals(1, comment.rating)

        commentService.removeVote(OperateOnContentRequest(commentId, 10, now()))
        comment = commentService.findById(commentId).data!!
    ...
    fun canBeRemovedOrActivated() {
        ...
        assertTrue(commentService.findById(1).data!!.isRemoved)

        commentService.activate(OperateOnContentRequest(1, 1, dateTime("2021-04-04T12:00:00")))
        assertTrue(commentService.findById(1).data!!.isActive)
        assertEquals(dateTime("2021-04-04T12:00:00"), commentService.findById(1).data!!.updatedAt)
