...
class PinRepositoryImplTest(
    ...
    fun isCorrectlyInitializedAndCanBeWorkedWith() {
        assertTrue(pinRepositoryImpl.list(mockk()).isNotEmpty())
    ...
    fun listsPinsByRoute() {
        val pins = pinRepositoryImpl.listForRoute("123", mockk())
        assertEquals(2, pins.size)
        assertTrue(pins.all { it.routeCode() == "123" })
        assertTrue(pinRepositoryImpl.listForRoute("1234", mockk()).all { it.routeCode() == "1234" })
    }
}